# Changelog
All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## 1.2.0 - 2026-07-31
### Changed
- Update generated FFI bindings and use build hooks for linking them

## 1.1.7 - 2026-06-11
### Changed
- Updated min sdk version to ^3.12.0
- Updated dependencies

## 1.1.6 - 2026-03-26
### Fixed
- Fixed launchd to use "dart install" instead of "dart pub global activate"

## 1.1.5 - 2026-03-17
### Changed
- Updated sodium to 4.0

## 1.1.4 - 2026-02-23
### Changed
- Updated dependencies
- Updated dart sdk to 3.11.0

## 1.1.3 - 2025-12-06
### Changed
- Updated dependencies
- Updated dart sdk to 3.10.0

## 1.1.2 - 2025-11-10
### Changed
- Updated dependencies
- Updated dart sdk to 3.9.0

## 1.1.1 - 2025-01-08
### Fixed
- Add terminal-notifier location to PATH in launchd setup

## 1.1.0 - 2024-12-02
### Added
- Add support for custom setup scripts
- Add terminal notifier to get notified of update result

## 1.0.1 - 2024-10-21
### Changed
- Enable automatic deployments
- Add timestamps to logs

### Fixed
- Fixed launchctl registration
- Fixed missing directory in git tool

## 1.0.0 - 2024-10-21
### Added
- Initial release
